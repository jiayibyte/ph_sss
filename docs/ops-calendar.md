...
| 时间 / 触发 | 谁 | 事项 | 怎么做 |
|---|---|---|---|
| **立刻** | 用户 | GSC 请求编入索引 12 个考试页 | GSC → 网址检查 → 逐个「请求编入索引」：`/prc-board-exam-schedule/`、`/nursing-board-exam-schedule/`、`/let-`、`/criminology-`、`/cpa-`、`/civil-engineering-`、`/physician-`、`/pharmacy-`、`/midwifery-`、`/psychometrician-`、`/radtech-`、`/medtech-board-exam-schedule/`。IndexNow 已在部署时推给 Bing/Yandex，Google 只认 GSC。 |
