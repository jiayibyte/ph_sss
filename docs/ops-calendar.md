...
| **2026-09-26（自动）** | 代码 | NCR-28 生效日 | 已自动：DOLE 公布 9 月 26 日生效（`wages/2026.json` 的 `ncr.upcoming.effectivity`，来源 BusinessMirror 9/17 转述 DOLE），服务器 00:05 夜间重建会把最低工资页、日薪页、JSON 数据切到 ₱755/₱718，sitemap lastmod 同步变并 ping IndexNow。**只需** 9/26 当天看一眼 https://nwpc.dole.gov.ph/ncr/ 有没有法院对 NCR-28 的 TRO/禁制令；有 → 把 `ncr.upcoming.effectivity` 与 regions[ncr].upcoming.effectivity 改回 null 并部署。之后找时间把 `ncr.in_force` 正式换成 NCR-28（纯整理，不影响显示） |
| **2026-12-01（自动）** | 代码 | Bicol / BARMM 第二档生效 | 已自动：`upcoming.tranche: true`，夜间重建当天把 Region V 切到 ₱480、BARMM 切到 ₱461/₱436/₱436/₱426（表格显示 "tranche December 1, 2026"）。不用改数据；之后年度整理时可把 tiers 直接换成第二档、删掉 upcoming |
...
| **2026-12-31** | 代码 | Pag-IBIG 房贷利率到期 | 官网写的是 "rates until December 31, 2026"（常规 6.5%–9.75%、促销 4.5%/5.75% 前 3 年）。1 月 1 日起页面与计算器会自动显示「利率可能已变，去官网核对」提示（`rates_valid_until` 过期判断，lastmod 同步变）。年底前后看 Availmentofnewloan.html 的新利率图，更新 `housing_loan.regular_rates` / `promo` / `rates_valid_until` 后提示自动消失。官网的利率表是图片（HL_InterestRates_01/02），要用浏览器看图 |
| **每季度 / Pag-IBIG 发新通告时** | 代码 | Pag-IBIG MPL / Calamity Loan 条款 | 09-23 新页 `/pagibig-salary-loan-calculator/`：数据在 `pagibig/<年>.json` 的 `short_term_loans`（Circular 469：储蓄 90%、月息 1.4583% 递减余额含宽限期、12/24/36 月、宽限 2 月、12 次缴存、4 次还款+第 6 月可续贷；Circular 470：年息 5.95%、宽限 3 月、灾害宣布后 90 天内申请；日罚 1/20 of 1%）。每季度打开 member_cashloans_mpl.html / member_cashloans_calamity.html 看利率或期限有没有变（Calamity 利率由 Interest Rate Setting Committee 调整）；官网 PDF 用内置浏览器 + pdf.js 读（直接 curl 会被 reCAPTCHA 挡） |
| **每年 1 月** | 代码 | 所得税页年检 | `/income-tax-calculator/`、`/freelancer-tax-calculator/`：TRAIN 2023 起税表无后续台阶，2026 沿用；每年 1 月确认 BIR 有没有新的 RR 改 Annex E 预扣表、₱90,000 免税额、8% 选项或 3% percentage tax（EOPT 已定 3%）；数据在 `tax/<年>.json`（`withholding_tables`、`annual_brackets`、`self_employed`），单测会校验每档衔接 |
| **2027 年 1 月（DBM 发 NBC 后）** | 代码 | 公务员薪级表换年 | `/salary-grade-table/` 已按日期自动切到 2027 档（EO 64 第四档，`ssl/2026.json` 的 `tranches`，1 月 1 日夜间重建自动切、lastmod 同步变）。要人做的：① DBM 发 2027 年实施通告（NBC，通常 1 月）后核对 2027 表 258 格并把 `issuance`/`issuance_url` 填上；② FY 2027 GAA 通过后核对 PERA ₱2,000、服装 ₱7,000、医疗 ₱7,000、年中/年终奖规则，复制为 `ssl/2027.json` 并更新 `last_verified`；③ EO 64 只到 2027 档，2028 起要看有没有新 EO/法律 |
| **每季度** | 代码 | 首次求职者（RA 11261）指南费用 | `/first-time-jobseeker/` 里的常规收费：NBI ₱130 + ₱30 e-payment、PNP ₱150 + 手续费、PSA ₱155 / Serbilis ₱330——去 nbi.gov.ph、pnpclearance.ph、psaserbilis.com.ph 核一次；DOLE-BLE 站（ble.dole.gov.ph）对脚本 403，用内置浏览器看有没有新的 JOG/表格 |
